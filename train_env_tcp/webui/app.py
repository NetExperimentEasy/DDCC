    ...
    def parser_redis_data(self, channel: list):
        ...
        for msg in msg_stream:
            ...
            if msg["type"] == "pmessage":
                ...
                if len(data_list) == 0:
                    continue
                # print(current_channel, data_list)
                if current_channel == "mininet":
                    ...
                    if len(data_list) == 3:  # 长度3 是 流执行完的统计信息，流结束
                        continue
                elif current_channel.startswith("rlccstate_"):
                    if self.reset_flag == None:
                        self.reset_flag = current_channel
                    if self.reset_flag != current_channel:
                        self.reset_flag = current_channel
                    ...
                    if len(data_list) > 6:
                        ...
                        self.datas[channel[0]].throughput.append(
                            np.float32(data_list[1])/1024),
                        self.datas[channel[0]].delivered_rate.append(
                            np.float32(data_list[0])/1024),
                        self.datas[channel[0]].rtt.append(
                            np.float32(data_list[3])/1024),  # 2 minrtt 3 rtt
                        self.datas[channel[0]].loss.append(
                            np.float32(data_list[4])),
...
if __name__ == "__main__":
    ...
    datasource = DataSource(channels=["rlccstate_*"], redis_ip="127.0.0.1", redis_port=6379)
